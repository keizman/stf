...
module.exports.builder = function(yargs) {
  ...
  return yargs
    ...
    , type: 'number'
    , default: envNumber('SCREEN_FRAME_RATE', -1)
    })
    .option('screen-jpeg-quality', {
      describe: 'The JPG quality to use for the screen.'
    , type: 'number'
    , default: envNumber('SCREEN_JPEG_QUALITY', 80)
    })
    ...
    .option('need-scrcpy', {
      describe: 'Use scrcpy for screen streaming instead of minicap. ' +
        'Provides better quality H.264 video stream.'
    , type: 'boolean'
    , default: false
    })
    .option('screen-reset', {
...
module.exports.handler = function(argv) {
  ...
  function run() {
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
        ...
        .concat(argv.needScrcpy ? ['--need-scrcpy'] : [])
    ...
    , procutil.fork(path.resolve(__dirname, '..'), [
      ...
      // image processor - connect directly to storage-temp to avoid proxy issues
    , procutil.fork(path.resolve(__dirname, '..'), [
      ...
      // apk processor - connect directly to storage-temp to avoid proxy issues
    , procutil.fork(path.resolve(__dirname, '..'), [
          'storage-plugin-apk'
