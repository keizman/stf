...
module.exports = syrup.serial()
  ...
  .define(function(options, adb, minicap, scrcpy, display, screenOptions) {
    ...
    log.info('Scrcpy mode: %s', options.needScrcpy ? 'enabled' : 'disabled')
    ...
    return createServer()
      .then(function(wss) {
        // Initialize scrcpy client if needed
        var scrcpyClient = null
        var scrcpyStarted = false
        
        if (options.needScrcpy) {
          log.info('Creating scrcpy client for device %s', options.serial)
          scrcpyClient = scrcpy.createClient({
            serial: options.serial,
            maxSize: 600,
            bitrate: 8000000
          })
          
          var videoDataCount = 0
          
          // Setup scrcpy video data handler ONCE here
          // Broadcast video data to ALL connected WebSocket clients
          scrcpyClient.on('video-data', function(data) {
            videoDataCount++
            log.info('[STREAM] Received video-data event #%d, size: %d bytes', videoDataCount, data.length)
            
            // Broadcast to all clients in broadcastSet
            var clients = broadcastSet.keys()
            log.info('[STREAM] Broadcasting to %d clients', clients.length)
            
            if (clients.length > 0) {
              clients.forEach(function(id) {
                var clientInfo = broadcastSet.get(id)
                if (!clientInfo) {
                  log.warn('[STREAM] Client %s not found in broadcastSet', id)
                  return
                }
                
                var ws = clientInfo.ws
                if (ws && ws.readyState === WebSocket.OPEN) {
                  ws.send(data, { binary: true }, function(err) {
                    if (err) {
                      log.error('[STREAM] Failed to send video data to client %s: %s', id, err.message)
                    } else {
                      // Success - don't log every frame, too verbose
                      if (videoDataCount % 30 === 0) {
                        log.info('[STREAM] Sent frame #%d to client %s', videoDataCount, id)
                      }
                    }
                  })
                } else {
                  log.warn('[STREAM] WebSocket for client %s not ready, state: %d', id, ws ? ws.readyState : -1)
                }
              })
            } else {
              log.warn('[STREAM] No clients connected to receive video data')
            }
          })
          
          scrcpyClient.on('end', function() {
            log.warn('[STREAM] Scrcpy END event received')
            scrcpyStarted = false
          })
          
          scrcpyClient.on('error', function(err) {
            log.error('[STREAM] Scrcpy ERROR event: %s', err.message)
          })
        }
        ...
        broadcastSet.on('nonempty', function() {
          if (options.needScrcpy && !scrcpyStarted) {
            // Start scrcpy instead of minicap
            log.info('[STREAM] BroadcastSet nonempty - starting scrcpy client')
            scrcpyClient.start()
              .then(function(deviceInfo) {
                scrcpyStarted = true
                log.info('[STREAM] Scrcpy started successfully: %s (%dx%d)', 
                  deviceInfo.name, deviceInfo.width, deviceInfo.height)
                log.info('[STREAM] Waiting for video data events...')
              })
              .catch(function(err) {
                log.error('[STREAM] Failed to start scrcpy: %s', err.message)
              })
          } else {
            frameProducer.start()
          }
        ...
        broadcastSet.on('empty', function() {
          if (options.needScrcpy && scrcpyStarted) {
            log.info('[STREAM] BroadcastSet empty - stopping scrcpy client')
            scrcpyClient.stop()
              .then(function() {
                scrcpyStarted = false
                log.info('[STREAM] Scrcpy stopped successfully')
              })
          } else {
            frameProducer.stop()
          }
        ...
        wss.on('connection', function(ws) {
          ...
          ws.on('message', function(data) {
            ...
            if (match) {
              ...
              case 'on':
                log.info('[WEBSOCKET] Client %s sent "on" message', id)
                if (options.needScrcpy) {
                  // For scrcpy, store ws reference for video broadcasting
                  log.info('[WEBSOCKET] Inserting client %s into broadcastSet for scrcpy', id)
                  broadcastSet.insert(id, {
                    ws: ws,
                    onStart: function() { return Promise.resolve() },
                    onFrame: function() { return Promise.resolve() }
                  })
                  log.info('[WEBSOCKET] BroadcastSet now has %d clients', broadcastSet.keys().length)
                } else {
                  broadcastSet.insert(id, {
              ...
              case 'size':
                ...
                // scrcpy doesn't support dynamic size changes
                break
              }
            }
          })
        ...
        lifecycle.observe(function() {
          if (options.needScrcpy && scrcpyClient) {
