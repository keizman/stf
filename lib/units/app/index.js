...
module.exports = function(options) {
  ...
  // Serve Broadway WASM file at root for H.264 decoding
  app.get('/avc.wasm', function(req, res) {
    var wasmPath = pathutil.resource('build/avc.wasm')
    if (fs.existsSync(wasmPath)) {
      res.type('application/wasm')
      res.sendFile(wasmPath)
    } else {
      res.status(404).send('avc.wasm not found')
    }
  })
